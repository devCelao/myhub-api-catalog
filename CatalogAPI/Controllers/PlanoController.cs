        ...
        [FromBody] PlanoRequest request)
    {
        ...
        if (request.CodPlano != codPlano)
        {
            return BadRequest(new ApiResponse<PlanoDto>
            {
                ...
                Errors = [new ApiError("VALIDATION", "Codigo do plano na URL não corresponde ao codigo no corpo da requisicao.")]
            });
        }
        ...
        [FromBody] List<string> codServicos)
    {
        if (codServicos == null || codServicos.Count == 0)
        {
            return BadRequest(new ApiResponse<PlanoDto>
            {
                ...
                Errors = [new ApiError("VALIDATION", "Lista de servicos não pode ser vazia.")]
            });
        }
